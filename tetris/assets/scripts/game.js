...
var nbNext = 1;
// value in the grid of a cell occupied by a block of a fallen tetromino
var blockValue = 1;
...
// contains the list the nbNext next tetrominoes to display
var queue = [];
...
// The tetromino that represents the falling tetromino
function Tetromino() {
  ...
  this.sprites = [];
  // list of the cells occupied by the tetromino
  this.cells = [];
  this.center = [0, 0];
  ...
  this.materialize = function(c_x, c_y, inGame) {
    ...
    this.sprites = [];
    // Check the occupied cells where the tetromino appears. If yes -> game over
    var conflict = false;
    for (var i = 0; i < blocksPerTetromino; i++) {
      // Compute the coordinates of each block of the tetromino, using the offset
      ...
      if (inGame) {
        ...
        // 1 for blocks of current tetromino, 2 for fallen blocks
...
Game.create = function() {
  ...
  // 2D array of blocksX*blocksY cells corresponding to the playable scene;
  // contains 0 for empty,
  // 1 if there is block already
  // and 2 if there is a block from a fallen tetromino
  scene = [];
  ...
  // Create ground for the tetrominos to fall on
  ...
  // Left Menu text for the score, number of lines, next tetromino
  ...
  var linesTitle = game.add.bitmapText(gameWidth + 50, 200, 'videogame', 'Lines', 20);
  linesText = game.add.bitmapText(gameWidth + 50, 250, 'desyrel', '0', 40);
...
// add tetrominos to queue, add new tetromino to the queue, display next tetromino
...
// Move the block of the falling tetromino left, right or down
...
function rotate(block, dir) {
  ...
  // Adjust for the phaser JS coordinates system to Cartesian coordinates
  offset_y = -offset_y;
...
// Check if the desired move (slide or rotate) doesn't conflict with something
function canMove(coordinatesCallback, dir) {
  if (pauseState) {
    return false;
  }
  for (var i = 0; i < tetromino.cells.length; i++) {
    // return coords in terms of cells
...
// Move (slide or rotate) a tetromino
...
// Remove the blocks from a filled line
function cleanLine(line) {
  var delay = 0;
  for (var k = 0; k < blocksX; k++) {
    // Animation to send the removed blocks flying to the top
...
function fall() {
  ...
  if (canMove(slide, "down")) {
    ...
    // If it cannot move down, it means it is touching fallen blocks;
    // it's time to see if a line has been completed
    // and to spawn a new falling tetromino
    var lines = [];
    for (var i = 0; i < tetromino.cells.length; i++) {
      // Make a set of the y coordinates of the falling tetromino;
      // the lines corresponding to those y coordinates will be
